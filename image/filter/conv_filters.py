...
def laplacian(
    ...
    if not isinstance(scale, (int, float)):
        raise WrongArgumentsType(
            "Provided value of scale does not have the accurate type"
        )

    if not isinstance(delta, (float, int)):
        raise WrongArgumentsType(
            "Provided value of delta does not have the accurate type"
        )
    ...
    try:
        ...
        new_im.image = cv2.Laplacian(
            new_im.image, int(kernel_size[0]), float(scale), float(delta),
            border_actual
        )
        new_im.update_file_stream()
        new_im.set_loader_properties()
